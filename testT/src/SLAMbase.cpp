...
{
    cv::Point3f p; // 3D 点
    p.z = static_cast<float>(double(point.z ) / camera.scale);
    p.x = static_cast<float>((point.x - camera.cx) * p.z / camera.fx);
    p.y = static_cast<float>((point.y - camera.cy) * p.z / camera.fy);
    return p;
}
